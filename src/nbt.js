...
// a byte-swapped read hits an invalid tag type or runs off the end within a few
// hundred bytes, so parsing both ways tells java and bedrock apart
export async function readNBT(input, { littleEndian, skip, only } = {}) {
  ...
  skip = keySet(skip)
  only = keySet(only)
  if (littleEndian !== undefined) return parseNBT(bytes, littleEndian, skip, only).root

  let first = null, partial = null
  for (const le of [false, true]) {
    let out
    try {
      out = parseNBT(bytes, le, skip, only)
    } catch (err) {
      first ??= err
      continue
    }
    if (out.end === bytes.length) return out.root
    partial ??= out
  }
  if (partial) return partial.root
  throw new Error("not NBT data, in either endianness", { cause: first })
}

const keySet = v => v == null ? undefined : v instanceof Set ? v : new Set(typeof v === "string" ? [v] : v)

function parseNBT(bytes, le, skip, only) {
    ...
    switch (type) {
      ...
      case TAG.COMPOUND: {
        ...
        for (;;) {
          ...
          if (t === TAG.END) break
          const name = string()
          if (skip?.has(name) || (root && only && !only.has(name))) skipPayload(t)
  ...
  string()
  return { root: payload(TAG.COMPOUND, true), end: o }
}

// taking a parsed root as well as bytes keeps format sniffing to one parse
export const rootOf = (input, opts) =>
  input && typeof input === "object" && !(input instanceof ArrayBuffer) && !ArrayBuffer.isView(input)
    ? input
    : readNBT(input, opts)
